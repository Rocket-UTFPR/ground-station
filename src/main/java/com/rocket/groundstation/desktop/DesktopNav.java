...
public class DesktopNav {
    ...
    private FileInFrameCtrl fileInFrameCtrl;
    ...
    public MapInFrame openMapInFrame(){
        if(mapInFrCtrl==null) mapInFrCtrl = new MapInFrameCtrl(new MapInFrame(), settings, srs.getDecodedDataDispatcher(), ls);
    ...
    public SerialMonitorInFrame openSerialMonitorInFrame(){
        if(SerialMonitorInFrCtrl==null) SerialMonitorInFrCtrl = new SerialMonitorInFrameCtrl(new SerialMonitorInFrame(), settings, srs);
        return SerialMonitorInFrCtrl.getSerialMonitorInFrame();
    }
    
    public FileInFrame openFileInFrame(){
        if(fileInFrameCtrl==null) fileInFrameCtrl = new FileInFrameCtrl(new FileInFrame(), new DataFileWriter(srs.getRawDataDispatcher(), srs.getDecodedDataDispatcher()));
        return fileInFrameCtrl.getFileInFrame();
    }
